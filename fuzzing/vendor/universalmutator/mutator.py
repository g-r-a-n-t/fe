...
try:
    # Optional dependency used only for `--comby` mode.
    # The upstream package imports it unconditionally; we make it optional so
    # the regex-based mutator works in minimal/offline environments.
    from comby import Comby  # type: ignore
except ModuleNotFoundError:  # pragma: no cover
    Comby = None  # type: ignore
...
def mutants_comby(source, ruleFiles=None, mutateTestCode=False, mutateBoth=False,
    ...
    if Comby is None:
        raise ModuleNotFoundError(
            "universalmutator `--comby` mode requires the `comby` Python package. "
            "Install it or run without `--comby`."
        )
    comby = Comby()
