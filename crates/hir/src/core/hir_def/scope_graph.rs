...
mod tests {
    ...
    fn item_tree() {
        ...
        let items: Vec<_> = scope_graph
            .items_dfs(&db)
            .filter(|item| !matches!(item, ItemKind::Use(use_) if use_.is_embed_use(&db)))
            .collect();
    ...
    fn enum_record_fields() {
        ...
        let enum_ = scope_graph
            .children(root)
            .find(|scope| !matches!(scope.item(), ItemKind::Use(use_) if use_.is_embed_use(&db)))
            .unwrap();
        assert!(matches!(enum_.item(), ItemKind::Enum(_)));

        let variant = scope_graph.children(enum_).next().unwrap();
        assert!(matches!(variant, ScopeId::Variant(..)));
    ...
    fn msg_variant_fields() {
        ...
        // Skip the Use item (embed) and find the struct
